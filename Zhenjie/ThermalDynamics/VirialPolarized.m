function [ KappaTilde, PTilde, TTilde, Z_vec ] = ...
    VirialPolarized(  PTildeMin, PTildeMax, LogPoints , order )
...
b1=1/2;
b2ref = -sqrt(2)/8/2;
b3ref = 1/(3^(5/2))/2; % Phys. Rev. Lett. 102, 160401 (2009)
b4ref = -1/(4^(5/2))/2; % Science 335, 563 (2012)
...
PTildeAll = 10*pi/(6*pi^2)^(2/3).* (b1.*Z_vec + b2 .* Z_vec.^2 + b3 .* Z_vec.^3 + b4 * Z_vec.^4)...
    ./ (b1.*Z_vec + 2*b2 .* Z_vec.^2 + 3*b3 .* Z_vec.^3 + 4*b4 * Z_vec.^4).^(5/3);

KappaTildeAll = (6*pi^2)^(2/3)/(6*pi) .* (b1.*Z_vec + 4*b2 .* Z_vec.^2 + 9*b3 .* Z_vec.^3 + 16*b4 * Z_vec.^4)...
    ./(b1.*Z_vec + 2*b2 .* Z_vec.^2 + 3*b3 .* Z_vec.^3 + 4*b4 * Z_vec.^4).^(1/3);

TTildeAll = 4*pi ./ (12* pi^2 * (b1.*Z_vec + 2*b2 .* Z_vec.^2 + 3*b3 .* Z_vec.^3 + 4*b4 * Z_vec.^4)).^(2/3);
